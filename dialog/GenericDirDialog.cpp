                                ...
                                const wxString& name)
{
    ...
    m_dirCtrl = new wxGenericDirCtrl(this, ID_DIRCTRL,
                                     ...
                                     wxSize(300, 200),
                                     dirStyle);
    m_dirCtrl->SetFont(wxSystemSettings::GetFont(wxSYS_OEM_FIXED_FONT));
...
//    topsizer->Add(check, wxSizerFlags(flagsBorder2).Right());
    ...
    // 2) text ctrl
    m_input = new wxTextCtrl( this, ID_TEXTCTRL, m_path, wxDefaultPosition, wxDefaultSize, wxBORDER_THEME);
	m_input->SetForegroundColour(wxColour(60, 60, 60));
	m_input->SetFont(*_gViewFont);//wxSystemSettings::GetFont(wxSYS_OEM_FIXED_FONT));
